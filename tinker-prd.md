...
- First run: after sign-in (§2.8), user picks a folder. Picked folder becomes the session's OpenCode `cwd`.
...
- First-run flow: sign-in screen precedes folder picker. After sign-in, a `users` row is created (`id, provider, provider_user_id, display_name, avatar_url, created_at`). Session creation binds to the current user via FK.

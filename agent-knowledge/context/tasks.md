...
| 8.15 | Integration test: sign in as User A → pick folder F → send message → sign out → sign in as User B → folder F session NOT visible in switcher → User B picks folder F → new JSONL created under `.tinker/chats/<user-b-id>/`. Doc result in `docs/mvp-verification.md`. | S | 8.11, 2.7, 2.11 | review | TIN-88. Vitest integration test at `apps/desktop/src/renderer/identityScopingIntegration.test.ts` exercises all six steps against real `@tinker/memory` + `@tinker/bridge` with Node-fs + in-memory SQL fakes. Scenario doc at `docs/mvp-verification.md`. |

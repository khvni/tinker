...
| 10 | `@tinker/panes` workspace layout primitive | p1 | in progress | [[10-tinker-panes]] | `[2026-04-20]` 69 tests green (was 41). Back-compat shims dropped (`movePaneRelativeToPane`, `onDropPaneOnPane`, `findLayoutRoot`, `findTabContainingPane`). `classifyBodyDrop` promoted to public. Dockview → panes per-pane migration still live below. |

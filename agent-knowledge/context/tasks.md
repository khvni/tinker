...
| 2.6 | ~~First-run screen (post-sign-in)~~ · Superseded by TIN-187 — boot goes straight to the workspace + `<SelectFolderButton>` in the Chat composer replaces the FirstRun wizard entirely. | M | 2.2, 2.3, 2.4 | review | TIN-187 · `feat/m2/kill-first-run`. Sign-in re-enters via Settings post-MVP per D25 amendment. |
...
| 4.10 | Parity verification: side-by-side with OpenCode Desktop from 4.1. Checklist in PR description. | S | 4.9 | review | TIN-47 · PR #103. Audit doc: `docs/model-picker-parity.md`; out-of-scope gaps tracked in TIN-189. |
...
| 6.13 | Seed Memory demo scaffold from Paper IQ-0 into the actual empty-memory bootstrap: shared seed notes, real sidebar counts, Paper article default selection, and browser preview parity without `VITE_E2E`-only data. | M | 6.3, 6.10, 6.12 | review | TIN-207 · PR #125. Shared seed scaffold now drives bootstrap + browser preview + visual baseline. |
...
| TIN-99 | 11 | Device ↔ host-service split — scaffold `packages/host-service` + `packages/host-client` (step 1 + 6 of [[11-host-service]] migration plan): `createHostApp()`, intrinsic identity, PSK auth, `health.check`, `host.info`. Foundation only — no business logic yet. Reopened from D25 deferral by 2026-05-05 Glass parity audit. | review | Branch `devin/tin-99-device-host-service-split`. Steps 2–5 + 7 land in TIN-141 / TIN-218 / follow-ups. |
| TIN-141 | 11 | Migrate `@tinker/scheduler` under host-service — host-side scheduler engine + HTTP endpoints (step 2 of [[11-host-service]] migration plan). | not started | Depends on TIN-99 scaffold. Owner: separate session. |
| TIN-218 | 11 | Headless long-running task runtime + permission approval queue — durable approval objects, pause/resume across app restarts, audit log. | not started | Depends on TIN-99 scaffold. Glass parity blocker for headless mode. |

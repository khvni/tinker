...
- **How to apply**:
  ...
  - **[2026-04-22] Application note (TIN-208 / PR #106)**: deferred feature data/model work may merge ahead of its UI, but **deferred UI affordances may not**. Until the route split in TIN-203 lands, Playbook stays out of `packages/shared-types/src/pane.ts`, `apps/desktop/src/renderer/workspace/register-pane-renderers.tsx`, `apps/desktop/src/renderer/workspace/Workspace.tsx`, and `apps/desktop/src/renderer/workspace/components/Titlebar/Titlebar.tsx`. No titlebar button, no pane registration, no Workspace/App shell entry point.

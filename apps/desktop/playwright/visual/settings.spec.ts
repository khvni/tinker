...
test.describe('settings pane', () => {
  test('settings: opens from sidebar and renders Account + Connections', async ({ page }) => {
    await page.goto('/');
    await waitForAppReady(page);
    await page.click('[aria-label="Settings"]');
    await page.waitForSelector('text=Account', { timeout: 5_000 });
    await captureAndCompare(page, 'settings-pane');
  });
});
